...
-- nothing else notices since our own in-memory playback_tracking
-- bookkeeping is untouched -- the bot still thinks it's playing, it just
-- silently isn't anymore. Fires on every ready (first boot is a no-op
-- since playback_tracking is empty then) and re-attaches whatever this
-- process still thinks is actively playing.
